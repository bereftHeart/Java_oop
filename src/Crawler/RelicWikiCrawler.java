	...
	public void start() throws Exception{
		...
		for (Element table : tables) {
			...
			for (List<Element> cell : table_datas) {
				...
				if (relicName.length()<5 ) continue;
		
				String relicDescription;
				Element firstAtag = cell.get(headerIndex).selectFirst("a");
				
				relicDescription = firstAtag == null ? "Chưa có thông tin chi tiết" : Wiki.getEntityDetails(rootURL + firstAtag.attr("href"));
				
				Map<String, String> relicAdditionalInfo = new HashMap<String, String>();
				for (int i = 1 + headerIndex; i<cols; i++) {
					if (cell.get(i)==null || cell.get(i).text().equals("")) continue;
					relicAdditionalInfo.put(table_headers.get(i),Wiki.getComplexInnerTextOfElement(cell.get(i)));
				}
